namespace Space
{
    ...
    public class GameManager : MasterGameManager
    {
        ...
        public void NextLevel()
        {
            ...
            // Find the current scene in our space game scenes list
            int currentIndexInList = spaceGameScenes.IndexOf(currentSceneIndex);

            if (currentIndexInList >= 0 && currentIndexInList < spaceGameScenes.Count - 1)
            {
                // Load next scene in the space game list
                int nextSceneIndex = spaceGameScenes[currentIndexInList + 1];
                Time.timeScale = 1f;
